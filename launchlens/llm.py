...
# Snapshot reference. Same object as get_settings() (lru_cached) — keep as a
# convenience for tests that mutate settings via monkeypatch. Live reads use
# get_settings() inline so reload_settings() at runtime takes effect.
...
def _get_cache():
    ...
    try:
        ...
        _cache = Cache(
            str(cache_dir),
            size_limit=int(settings.cache_size_limit_gb * 1024**3),
        )
    except ImportError:
...
class OllamaProvider:
    ...
    def probe(cls, base_url: str | None = None) -> list[str]:
        ...
        # Strip "/v1" suffix correctly (rstrip treats arg as char-set, not suffix)
        if url.endswith("/v1"):
            url = url[:-3]
...
class SarvamProvider:
    """Remote Sarvam-105B (OpenAI-compatible)."""
    ...
    def __init__(self) -> None:
        if not get_settings().sarvam_api_key:
            raise MissingAPIKey("SARVAM_API_KEY not configured.")
    ...
    async def complete(self, *, system: str, user: str, temperature: float,
        ...
        cost = (
            input_toks * settings.sarvam_input_price_per_mtok / 1_000_000
            + output_toks * settings.sarvam_output_price_per_mtok / 1_000_000
        )
        return CompletionResult(
            text=text, provider=self.name, model=self.model,
...
class ClaudeProvider:
    """Remote Claude (Anthropic Messages API)."""
    ...
    def __init__(self) -> None:
        if not get_settings().anthropic_api_key:
            raise MissingAPIKey("ANTHROPIC_API_KEY not configured.")
    ...
    async def complete(self, *, system: str, user: str, temperature: float,
        ...
        cost = (
            in_toks * settings.claude_input_price_per_mtok / 1_000_000
            + out_toks * settings.claude_output_price_per_mtok / 1_000_000
        )
        return CompletionResult(
            text=text, provider=self.name, model=self.model,
...
def _engine_setting() -> str:
    ...
    if env in _VALID_ENGINES:
        return env
    cfg = (get_settings().launchlens_engine or "auto").lower().strip()
    return cfg if cfg in _VALID_ENGINES else "auto"


def _model_for_route(route: LLMRoute, model_override: str | None = None) -> str:
    if model_override:
...
def reset_provider_cache() -> None:
    """Clear cached availability probes. Call after `ollama pull` or restart."""
...
def select_provider(
    ...
    *,
    ...
    ``model_override`` is honored only for the LOCAL/Ollama path.
    """
    ...
    if requested == "local":
        model = _model_for_route(route, model_override)
    ...
    if _ollama_available(model):
        return OllamaProvider(model=model)
    if route == LLMRoute.SARVAM and settings.sarvam_api_key:
        return SarvamProvider()
    if route == LLMRoute.CLAUDE and settings.anthropic_api_key:
        return ClaudeProvider()
...
async def complete(
    ...
    provider = select_provider(
        route, engine_override=engine_override, model_override=model_override,
    )
...
def effective_max_concurrent(
    ...
    """Return the appropriate concurrency limit for the active provider."""
    settings = get_settings()
    try:
        provider = select_provider(
            LLMRoute.SARVAM,
        ...
        return settings.llm_max_concurrent_local
    return (
        settings.llm_max_concurrent_local
        if provider.name in ("ollama", "mock")
        else settings.llm_max_concurrent_remote
    )
...
def estimate_cost(
    ...
    if pname in ("ollama", "mock"):
        return 0.0, pname
    if pname == "sarvam":
        cost = (
            calls * avg_input_tokens * settings.sarvam_input_price_per_mtok / 1_000_000
            + calls * avg_output_tokens * settings.sarvam_output_price_per_mtok / 1_000_000
        )
        return round(cost, 4), pname
    if pname == "claude":
        cost = (
            calls * avg_input_tokens * settings.claude_input_price_per_mtok / 1_000_000
            + calls * avg_output_tokens * settings.claude_output_price_per_mtok / 1_000_000
        )
        return round(cost, 4), pname
    return 0.0, pname
...
__all__ = [
    "LLMRoute", "LLMError", "MissingAPIKey", "LocalModelUnavailable",
    "CompletionResult", "LLMProvider",
    "MockProvider", "OllamaProvider", "SarvamProvider", "ClaudeProvider",
    "LLMUsageTracker", "get_usage_tracker",
    "select_provider", "reset_provider_cache", "route_for_agent",
    "complete", "estimate_cost", "effective_max_concurrent",
]
